...
Процедура ЗапуститьПриложение(Заголовок = "", ШиринаОкна = 800, ВысотаОкна = 600) Экспорт
    ...
    ОснованяФормаПриложения.ДвойнаяБуферизация = Ложь;
    ...
    //
    // ПЭУ = ОснованяФормаПриложения.ЭлементыУправления.Добавить(Интерфейс.ПользовательскийЭлементУправления());
    // ПЭУ.Размер = Интерфейс.Размер(200, 200);
    // ПЭУ.Центр();
    // ПЭУ.ЦветФона = Интерфейс.Цвет().Синий;
    // ПЭУ.Текст = "Это ПользовательскийЭлементУправления";
    //
    ...
    Для Каждого ТекущийОбъектКЗ из СортировкаСоотвествия(ИгровыеОбъекты, "СлойОтрисовки") Цикл
        ...
        Если ТекущийОбъект.Видимость = Истина Тогда
            ...
            Если ТекущийОбъект.ОтображатьГраницы = Истина Тогда
                ...
                СплошнаяКисть1 = Интерфейс.СплошнаяКисть(Интерфейс.Цвет().Серый);
                Холст.ЗаполнитьПрямоугольник(СплошнаяКисть1, ТекущийОбъект.КоординатаХ-1, 
