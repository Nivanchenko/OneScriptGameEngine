...
Процедура ПриСозданииОбъекта()
    ...
    ОтложеныеИгровыеСобытия = ИнициализацияТаблицыОтложеныхИгровыхСыбытий();

    ПодпискиНаИгровойЦикл = ИнициализацияПодписокНаИгровойЦикл();

КонецПроцедуры

Функция ИнициализацияТаблицыОтложеныхИгровыхСыбытий()
    ТаблицаЗначений = Новый ТаблицаЗначений();
    ТаблицаЗначений.Колонки.Добавить("СчетчикСобытий");
    ТаблицаЗначений.Колонки.Добавить("Делегат");
    ТаблицаЗначений.Колонки.Добавить("ВФоне");
    ТаблицаЗначений.Колонки.Добавить("ДопПараметры");
    ТаблицаЗначений.Колонки.Добавить("Новый"); 
    Возврат ТаблицаЗначений;  
КонецФункции

Функция ИнициализацияПодписокНаИгровойЦикл()
    ТаблицаЗначений = Новый ТаблицаЗначений();
    ТаблицаЗначений.Колонки.Добавить("ИгровойОбъект");
    ТаблицаЗначений.Колонки.Добавить("Делегат");
    ТаблицаЗначений.Колонки.Добавить("ВФоне");
    ТаблицаЗначений.Колонки.Добавить("ДопПараметры");
    ТаблицаЗначений.Колонки.Добавить("Новый"); 
...
Процедура ПодписатьНаИгровойЦикл(ИгровойОбъект, Делегат, ДопПараметры = Неопределено, ВФоне = Ложь) Экспорт
    ...
    НоваяПодписка = ПодпискиНаИгровойЦикл.Добавить();
...
Процедура УдалитьПодпискуНаИгровойЦикл(ИгровойОбъект) Экспорт
    СтрокиКУдалению = ПодпискиНаИгровойЦикл.НайтиСтроки(Новый Структура("ИгровойОбъект",  ИгровойОбъект));
    Для каждого СтрокаКУдалению Из СтрокиКУдалению Цикл
        ПодпискиНаИгровойЦикл.Удалить(СтрокаКУдалению);
    КонецЦикла;    
КонецПроцедуры

Процедура ОбработатьПодпискиНаИгровойЦикл() Экспорт
    
    ТекущиеПодпискиКОбработке = ПодпискиНаИгровойЦикл.НайтиСтроки(Новый Структура("Новый", Ложь));
    ...
    Для каждого ТекПодписка Из ПодпискиНаИгровойЦикл Цикл
...
Функция СоздатьФизику() Экспорт
    ...
    Возврат Новый Физика(ЭтотОбъект);
...
Функция ДобавитьИгровойОбъект(_ТипОбъекта = "", _СлойОтрисовки = 0, _СлойФизики = 0) Экспорт
    ...
    НовыйИгровойОбъект = Новый ИгровойОбъект(ЭтотОбъект, _ТипОбъекта, _СлойОтрисовки, _СлойФизики);
...
Процедура ОбработатьОтложенныеИгровыеСобытия() Экспорт
    ...
    ТекущиеСтрокиКОбработке = ОтложеныеИгровыеСобытия.НайтиСтроки(Новый Структура("Новый", Ложь));
