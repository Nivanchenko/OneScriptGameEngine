...
Функция ИнициализацияТаблицыОтложеныхИгровыхСыбытий()
    ...
    ТаблицаЗначений.Колонки.Добавить("МеткиСобытия");
    ТаблицаЗначений.Колонки.Добавить("Новое"); 
...
Процедура ДобавитьОтложенноеИгровоеСобытие(Делегат, ЗадержкаВИгровыхЦиклах = 0, ДопПараметры = Неопределено, ВФоне = Ложь, МеткиСобытия = "") Экспорт
    ...
    НовоеСобытие.МеткиСобытия = МеткиСобытия;
...
Процедура ОбработатьОтложенныеИгровыеСобытия() Экспорт
    ...
    ТекущиеСтрокиКОбработке = ОтложеныеИгровыеСобытия.НайтиСтроки(Новый Структура("Новое", Ложь));
